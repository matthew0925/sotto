...
記録を保存する瞬間に、本文と写真をフィールド識別子・長さで区切ったバージョン付き形式からSHA-256ハッシュを計算し、
...
`SottoShortcuts.swift`で4つのApp Intentsを提供します：

- **「そっとを開く」**：ホーム画面を開きます
- **「110番への電話画面」**：110番へ発信できるホーム画面を開きます
- **「見守りを開く」**：見守りチェックイン画面を開きます
- **「見守り情報を取得」**：期限超過後の送信可否・宛先・本文を、同じ見守りにつき1回だけオートメーションへ渡します

画面を開く3つのIntentは既存の`sotto://`ディープリンク（ウィジェットと同じ仕組み）を呼ぶだけで、
**画面を開くところまでしかしません**。実際に電話を発信する／見守りを開始するには、
...
- 多言語対応（在留外国人の被害者支援を視野に入れる場合、英語・やさしい日本語）

以下は実装済み：
- **相談窓口データの定期更新**：`resources.json`は`docs/resources.json`
  （GitHub Pages経由で配信）からアプリ起動時に`SupportResourceLoader.refreshFromRemote()`
  が取得し、成功時のみ端末内キャッシュを更新します。取得に失敗しても同梱JSON
  またはキャッシュ済みの内容を表示し続けるため、電話番号の更新にアプリの
  審査・リリースを待つ必要がなくなりました。
- **Dynamic Type（文字サイズ設定）対応**：本文・見出し系のテキストはすべて
  `.font(.system(.headline, ...))`のようなセマンティックなテキストスタイルへ
  移行済みです。SF Symbolsのアイコングリフや、見守りタイマーの巨大なカウントダウン
  数字（`52pt`固定）など、意図的にサイズ固定のままにしている箇所もあります。
- **ウィジェットのライブ更新**：`group.com.takashi.sotto`というApp Groupを
  Sotto/SottoWidgetExtension両ターゲットに追加し、`CheckInManager`が見守りの
  active/deadline状態をそこへ書き込みます。ウィジェット側は`Text(timerInterval:)`
  で残り時間を自動更新表示し、見守りの開始・終了時には`WidgetCenter.reloadTimelines`
  で即座に反映します。
